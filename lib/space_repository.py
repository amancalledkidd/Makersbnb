...
class SpaceRepository():
    ...
    def create(self, space):
        self._connection.execute('INSERT INTO spaces (name,address,price,description,user_id, image_url) VALUES(%s,%s,%s,%s,%s,%s) ', [
                                 space.name, space.address, space.price, space.description, space.user_id, space.image_url])
        return None
    # creates a space with name desc price etc

    def update(self, space):
        self._connection.execute("UPDATE spaces SET name = %s, address = %s, price = %s, description = %s, user_id = %s, image_url=%s WHERE id = %s",
